...
package com.myself.mylibrary.controller.eventbus;

import com.myself.mylibrary.controller.eventbus.handler.AsyncEventHandler;
import com.myself.mylibrary.controller.eventbus.handler.DefaultEventHandler;
import com.myself.mylibrary.controller.eventbus.handler.EventHandler;
import com.myself.mylibrary.controller.eventbus.handler.UIThreadEventHandler;
import com.myself.mylibrary.controller.eventbus.matchpolicy.DefaultMatchPolicy;
import com.myself.mylibrary.controller.eventbus.matchpolicy.MatchPolicy;
 ...
 * 事件类型和该事件对应的tag标识一个种类的事件{@see EventBusEventType},每一种事件对应有一个或者多个订阅者{@see EventBusSubscription}
 ...
 * {@see Subcriber}标识的函数,并且将它们以{@see EventBusEventType}为key,以{@see EventBusSubscription}
...
final class EventBus {
    ...
    /**
     * EventBusEventType-Subcriptions map
     */
    private final Map<EventBusEventType, CopyOnWriteArrayList<EventBusSubscription>> mSubcriberMap = new ConcurrentHashMap<EventBusEventType, CopyOnWriteArrayList<EventBusSubscription>>();

    /**
     * the thread local event queue, every single thread has it's own queue.
     */
    ThreadLocal<Queue<EventBusEventType>> mLocalEvents = new ThreadLocal<Queue<EventBusEventType>>() {
        protected Queue<EventBusEventType> initialValue() {
            return new ConcurrentLinkedQueue<EventBusEventType>();
        };
    };
    ...
    public void post(Object event) {
        post(event, EventBusEventType.DEFAULT_TAG);
    }
    ...
    public void post(Object event, String tag) {
        mLocalEvents.get().offer(new EventBusEventType(event.getClass(), tag));
     ...
     * @return
     */
    public Map<EventBusEventType, CopyOnWriteArrayList<EventBusSubscription>> getSubscriberMap() {
     ...
     * @return
     */
    public Queue<EventBusEventType> getEventQueue() {
    ...
    private class EventDispatcher {
         ...
         */
        private Map<EventBusEventType, List<EventBusEventType>> mCacheEventTypes = new ConcurrentHashMap<EventBusEventType, List<EventBusEventType>>();
        /**
        ...
        void dispatchEvents(Object aEvent) {
            Queue<EventBusEventType> eventsQueue = mLocalEvents.get();
         ...
         * @param aEvent
         */
        private void deliveryEvent(EventBusEventType type, Object aEvent) {
            Class<?> eventClass = aEvent.getClass();
            List<EventBusEventType> eventBusEventTypes = null;
            // 如果有缓存则直接从缓存中取
            if (mCacheEventTypes.containsKey(eventClass)) {
                eventBusEventTypes = mCacheEventTypes.get(type);
            } else {
                eventBusEventTypes = mMatchPolicy.findMatchEventTypes(type, aEvent);
                mCacheEventTypes.put(type, eventBusEventTypes);
            }

            for (EventBusEventType eventBusEventType : eventBusEventTypes) {
                handleEvent(eventBusEventType, aEvent);
            }
        }
         ...
         * 处理单个事件
         * 
         * @param eventBusEventType
         * @param aEvent
         */
        private void handleEvent(EventBusEventType eventBusEventType, Object aEvent) {
            List<EventBusSubscription> eventBusSubscriptions = mSubcriberMap.get(eventBusEventType);
